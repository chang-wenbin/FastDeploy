    ...
    if (q_head_dim == 576) {
        if (mla_use_wg4 && block_size == 32) {
            if constexpr (!std::is_same<CUTLASS_TYPE, cutlass::half_t>::value) {
                ...
                using ParamsType = Params<cutlass::half_t,
                                          cutlass::half_t,
                                          cutlass::half_t,
                                          int>;
                ParamsType params = {};
                params.Q = reinterpret_cast<cutlass::half_t*>(
                    const_cast<T*>(q.data<T>()));
                params.KV = reinterpret_cast<cutlass::half_t*>(
                    const_cast<T*>(latent_cache.data<T>()));
                params.O = reinterpret_cast<cutlass::half_t*>(
                    const_cast<T*>(out->data<T>()));
                ...
                params.batch_id_per_token =
                    const_cast<int*>(batch_id_per_token.data<int>());
                params.batch_ids = const_cast<int*>(batch_ids.data<int>());
                if (q_head_num <= 64) {
                    params.tile_ids_per_batch =
                        const_cast<int*>(tile_ids_per_batch.data<int>());
                } else {
                    params.q_tile_ids_per_batch =
                        const_cast<int*>(tile_ids_per_batch.data<int>());
                }
                params.num_blocks_x =
                    const_cast<int*>(num_blocks_x_device.data<int>());
                ...
                BatchMLAWithPagedKVCacheWG4Dispatched<576,
                                                      512,
                                                      half,
                                                      ParamsType,
            ...
            using ParamsType =
                Params<CUTLASS_TYPE, CUTLASS_TYPE, CUTLASS_TYPE, int>;
            ParamsType params = {};
            params.Q =
                reinterpret_cast<CUTLASS_TYPE*>(const_cast<T*>(q.data<T>()));
            params.KV = reinterpret_cast<CUTLASS_TYPE*>(
                const_cast<T*>(latent_cache.data<T>()));
            params.O =
                reinterpret_cast<CUTLASS_TYPE*>(const_cast<T*>(out->data<T>()));
