...
struct Params {
    ...
    alignas(
        16) float *m;  // [num_chunks, bsz * max_draft_token_num * head_num]
    alignas(
        16) float *d;  // [num_chunks, bsz * max_draft_token_num * head_num]
...
__global__ void __launch_bounds__(
    ...
    if (warp_group_idx == 0) {
        ...
        const int tile_id = mainloop_params.tile_ids_per_batch
                                ? mainloop_params.tile_ids_per_batch[block_id]
                                : 0;
        const int q_tile_id =
            mainloop_params.q_tile_ids_per_batch
                ? mainloop_params.q_tile_ids_per_batch[block_id]
                : 0;
...
cudaError_t BatchMLAWithPagedKVCacheKernelTraitsDispatched(
    ...
    typename CollectiveMainloop::Params mainloop_params =
        CollectiveMainloop::to_underlying_arguments(
            {make_layout(
             ...
             make_layout(make_shape(params.block_size,
                                    params.qk_head_dim,
    ...
    if (params.chunk_num > 1) {
        ...
        dim3 grids_merge(min(multiprocessor_count, params.token_num),
