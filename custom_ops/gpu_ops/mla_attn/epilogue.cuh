...
struct CollectiveEpilogue {
    ...
    using SharedStorage =
        cute::array_aligned<DTypeO, cute::cosize_v<SmemLayoutO>>;
    ...
    using TMA_O = decltype(make_tma_copy(
        GmemTiledCopyOTMA{},
        make_tensor(
            make_gmem_ptr(static_cast<DTypeO*>(nullptr)), ShapeT{}, StrideT{}),
        SmemLayoutO{},
        select<0, 1>(TileShape_PDV{}),
        _1{}));  // no mcast for O

    static constexpr int VEC_SIZE =
        cute::ceil_div(128, sizeof_bits_v<DTypeO>);  // 8
    ...
    using TiledCopyOAtom =
        cute::Copy_Atom<cute::UniversalCopy<cutlass::uint128_t>, DTypeO>;
    using TiledCopyOThrLayout = decltype(cute::make_layout(
        cute::make_shape(Int<NUM_ROWS>{}, Int<NUM_THREADS_PER_ROW>{}),
        LayoutRight{}));
    using TiledCopyOValLayout = decltype(cute::make_layout(
        cute::make_shape(_1{}, Int<VEC_SIZE>{}), LayoutRight{}));
    using TiledCopyO =
        decltype(make_tiled_copy(TiledCopyOAtom{},
    ...
    CUTLASS_DEVICE void store(Params const& epilogue_params,
        ...
        Tensor sO = make_tensor(make_smem_ptr(shared_storage.smem_o.data()),
                                SmemLayoutO{});
        auto smem_tiled_copy_O = make_tiled_copy_C(SmemCopyAtomO{}, tiled_mma);
        auto smem_thr_copy_O = smem_tiled_copy_O.get_thread_slice(thread_idx);

        Tensor tOrO_out = convert_type<DTypeO>(tOrO);
        Tensor taccOrO = smem_thr_copy_O.retile_S(
            tOrO_out);  // ((Atom,AtomNum), MMA_M, MMA_N)
        Tensor taccOsO =
        ...
        auto O_ptr =
            num_chunks == 1
                ? epilogue_params.O_ptr +
                      (start_token_idx * Ktraits::GROUP_SIZE + q_group_offset) *
                          o_stride_head
                : epilogue_params.O_ptr_tmp +
                      (((tile_idx * bsz + bid) * max_draft_token_num) *
                           Ktraits::GROUP_SIZE +
                       q_group_offset) *
                          o_stride_head;
        Tensor mO = make_tensor(make_gmem_ptr(O_ptr), epilogue_params.layout_O);
        Tensor gO = local_tile(
            mO, select<0, 1>(TileShape_PDV{}), make_coord(_, _0{}))(_, _, _0{});
        Tensor cO =
            make_identity_tensor(gO.shape());  // (O, D) -> (o_idx, d_idx)
