...
CUTLASS_DEVICE void mma_f16(const Params& mainloop_params,
    ...
    const int chunk_num_this_seq =
        cute::ceil_div(kv_len, mainloop_params.chunk_size);
    ...
    Tensor sK = make_tensor(make_smem_ptr(shared_storage.smem_kv.data()),
                            SmemLayoutK{});
    // todo: split rope/norope for sVt, not use s1~s2.
    Tensor sVt_s1 = make_tensor(make_smem_ptr(shared_storage.smem_kv.data()),
                                SmemLayoutVtOneStage{});
    Tensor sVt_s2 = make_tensor(
        make_smem_ptr(shared_storage.smem_kv.data() + Ktraits::NUM_PER_STAGE),
        SmemLayoutVtOneStage{});
    Tensor sPSS =
        make_tensor(make_smem_ptr(shared_storage.smem_p.data()), SmemLayoutP{});
    Tensor s_scale = make_tensor(
        make_smem_ptr(shared_storage.smem_scale.data()), SmemLayoutRow{});
    Tensor mM = make_tensor(make_gmem_ptr(mainloop_params.m_ptr),
                            mainloop_params.layout_MD)(
        tile_idx, _);  // (bsz * draft_token_num * num_head)
    Tensor mD = make_tensor(make_gmem_ptr(mainloop_params.d_ptr),
                            mainloop_params.layout_MD)(tile_idx, _);
    ...
    Tensor tScalesScale =
        s_scale(_, thread_idx % cutlass::NumThreadsPerWarpGroup);
    ...
    const int end_tile_idx =
        cute::ceil_div(min(start_len + mainloop_params.chunk_size, kv_len),
                       BLOCK_SHAPE_KV) -
        1;
    ...
    if (warp_group_idx == 1) {
        ...
        constexpr int n_masking_steps =
            !CAUSAL ? 1 : cute::ceil_div(BLOCK_SHAPE_Q, BLOCK_SHAPE_KV) + 1;
        auto col_limit_right = [&](int qo_idx) {
            return qo_idx + 1 + kv_len - qo_len;
        };
        bool is_first_step = true;
        // wait q
        consumer_wait(pipeline_q, smem_pipe_read_q);
        Tensor tSrS =
            partition_fragment_C(tiled_mma_qk, select<0, 1>(TileShape_QKD{}));
#pragma unroll 1
        for (int masking_step = n_masking_steps; kv_tile_idx >= start_tile_idx;
             --masking_step, --kv_tile_idx) {
            // wait kv
            consumer_wait(pipeline_kv, smem_pipe_read_kv);
            // gemm qk
            gemm</*init=*/true, /*wg_wait=*/0>(
                tiled_mma_qk,
                tSrQ,
                tSrK(_, _, _, smem_pipe_read_kv.index()),
                tSrS);
            // mask
            if (masking_step > 0) {
                Tensor cS =
                    cute::make_identity_tensor(select<0, 1>(TileShape_QKD{}));
                Tensor tScS = threadMmaQK.partition_C(cS);
#pragma unroll
                for (int i = 0; i < size(tSrS); ++i) {
                    int qo_idx = (get<0>(tScS(i)) + q_group_offset) /
                                 Ktraits::GROUP_SIZE;
                    ...
                    if constexpr (!CAUSAL) {  // Just masking based on col
                        ...
                        if (kv_idx >=
                            std::min(kv_len, col_limit_right(qo_idx))) {
                            tSrS(i) = AttentionUpdater::fill_value;
                        }
                    }
                }
            ...
            // update s (exp(s - m))
            Tensor scale_o =
                is_first_step ? attention_updater.update</*init=*/true>(tSrS)
            ...
            cutlass::arch::NamedBarrier::sync(
                Ktraits::NUM_MMA_THREADS,
                static_cast<int>(NamedBarriers::kWarpSchedulerWG1));

            attention_updater.rescale_o(tOrO, scale_o);

            // pv gemm
            if (smem_pipe_read_kv.index() == 0) {
                gemm</*init=*/false, /*wg_wait=*/0>(
                    tiled_mma_pv_ss, tOrP_CS2, tOrV1(_, _, _, _0{}), tOrO);
            } else {
                gemm</*init=*/false, /*wg_wait=*/0>(
                    tiled_mma_pv_ss, tOrP_CS2, tOrV2(_, _, _, _0{}), tOrO);
            }
            ...
            cutlass::arch::NamedBarrier::sync(
                Ktraits::NUM_MMA_THREADS,
                static_cast<int>(NamedBarriers::kWG1WG2Sync));
        }
        // release q
        ...
        // normalize
        Tensor scale_o =
            attention_updater.finalize(tSrS);  // warp reduce row sum
        if (chunk_num_this_seq == 1) {
            ...
            cutlass::arch::NamedBarrier::arrive(
                Ktraits::NUM_MMA_THREADS,
                static_cast<int>(NamedBarriers::kWarpSchedulerWG2));
            attention_updater.rescale_o(tOrO, scale_o);
        }
        ...
        if (chunk_num_this_seq > 1 &&
            thread_idx % 4 ==
                0) {  // 16 rows per warp, eg. t0->row0 row8，t4->row1 row9
            const int warp_idx = thread_idx / 32;
#pragma unroll
            for (int w_i = 0; w_i < 2; ++w_i) {
                const int token_group_idx = warp_idx * 16 +
                                            (thread_idx % 32) / 4 + 8 * w_i +
                                            q_group_offset;
                const int token_idx = token_group_idx / Ktraits::GROUP_SIZE;

                if (token_idx < qo_len) {
                    // const int head_idx = token_group_idx %
                    // Ktraits::GROUP_SIZE;
                    const int bid_offset = mainloop_params.max_draft_token_num *
                                           Ktraits::GROUP_SIZE;
                    const int write_idx = bid * bid_offset + token_group_idx;
                    mM(write_idx) =
                        static_cast<DTypeMD>(attention_updater.row_max(w_i));
                    mD(write_idx) =
                        static_cast<DTypeMD>(attention_updater.row_sum(w_i));
                }
            }
        }
...
CUTLASS_DEVICE void mma_f16_two_stages(const Params& mainloop_params,
    ...
    Tensor sVt_s2 = make_tensor(
        make_smem_ptr(shared_storage.smem_kv.data() + Ktraits::NUM_PER_STAGE),
        SmemLayoutVtOneStage{});
    Tensor sVt_s3 = make_tensor(make_smem_ptr(shared_storage.smem_kv.data() +
                                              2 * Ktraits::NUM_PER_STAGE),
                                SmemLayoutVtOneStage{});
    Tensor sVt_s4 = make_tensor(make_smem_ptr(shared_storage.smem_kv.data() +
                                              3 * Ktraits::NUM_PER_STAGE),
                                SmemLayoutVtOneStage{});
    Tensor sPSS =
        make_tensor(make_smem_ptr(shared_storage.smem_p.data()), SmemLayoutP{});
    Tensor mM = make_tensor(make_gmem_ptr(mainloop_params.m_ptr),
    ...
    Tensor tPsP = smem_thr_copy_P.partition_D(sPSS);
    Tensor tScalesScale =
        s_scale(_, thread_idx % cutlass::NumThreadsPerWarpGroup, _);
    ...
    if (warp_group_idx == 1) {
        ...
        // first qk gemm
        gemm</*init=*/true, /*wg_wait=*/0>(
            tiled_mma_qk, tSrQ, tSrK(_, _, _, smem_pipe_read_kv.index()), tSrS);
        // mask
        {
            Tensor cS =
                cute::make_identity_tensor(select<0, 1>(TileShape_QKD{}));
...
#pragma unroll
            for (int i = 0; i < size(tSrS); ++i) {
                int qo_idx =
                    (get<0>(tScS(i)) + q_group_offset) / Ktraits::GROUP_SIZE;
                int kv_idx = get<1>(tScS(i)) + kv_tile_idx * BLOCK_SHAPE_KV;
                if constexpr (!CAUSAL) {  // Just masking based on col
                    if (kv_idx >= kv_len) {
                        tSrS(i) = AttentionUpdater::fill_value;
        ...
        constexpr int n_masking_steps =
            CAUSAL ? cute::ceil_div(BLOCK_SHAPE_Q, BLOCK_SHAPE_KV) : 0;
        --kv_tile_idx;
        for (int masking_step = n_masking_steps; kv_tile_idx >= start_tile_idx;
             --masking_step, --kv_tile_idx) {
            Tensor tSrS = partition_fragment_C(tiled_mma_qk,
                                               select<0, 1>(TileShape_QKD{}));
            ...
            // gemm next qk
            gemm</*init=*/true, /*wg_wait=*/-1>(
                tiled_mma_qk,
                tSrQ,
                tSrK(_, _, _, smem_pipe_read_kv.index()),
                tSrS);
            attention_updater.rescale_o(tOrO);
            stage_div.fast_divmod(
                quotient, remainder, smem_pipe_read_kv_cur.index());
            // last pv gemm
            if (smem_pipe_read_kv_cur.index() == 0) {
                gemm</*init=*/false, /*wg_wait=*/-1>(
                    tiled_mma_pv_ss,
                    tOrP_CS2(_, _, _, remainder),
                    tOrV1(_, _, _, _0{}),
                    tOrO);
...
#pragma unroll
            ...
            // gather qk gemm res
            stage_div.fast_divmod(
                quotient, remainder, smem_pipe_read_kv.index());
            cute::copy(smem_tiled_copy_P, tPrP, tPsP(_, _, _, remainder));
            cute::copy(scale_o, tScalesScale(_, remainder));
            // r2s fence wgmma
            cutlass::arch::fence_view_async_shared();
        ...
        stage_div.fast_divmod(quotient, remainder, smem_pipe_read_kv.index());
        if (smem_pipe_read_kv.index() == 0) {
            gemm</*init=*/false, /*wg_wait=*/-1>(tiled_mma_pv_ss,
                                                 tOrP_CS2(_, _, _, remainder),
                                                 tOrV1(_, _, _, _0{}),
                                                 tOrO);
        ...
        if (chunk_num_this_seq == 1) {
            ...
            cutlass::arch::NamedBarrier::arrive(
                Ktraits::NUM_MMA_THREADS,
                static_cast<int>(NamedBarriers::kWG1WG2LastSync));
            attention_updater.rescale_o(tOrO);
        }
        // WG1 write m,d back to gmem
        if (chunk_num_this_seq > 1 &&
...
template <typename Ktraits,
          ...
          typename MainloopPipeline,
          typename MainloopPipelineQ,
          typename MainloopPipelineQK,
          typename PipelineState,
          typename PipelineStateQ,
          typename PipelineStateQK,
          typename SharedStorage,
          typename AttentionUpdater>
CUTLASS_DEVICE void mma_qk_one_stages(const Params& mainloop_params,
    ...
    Tensor sPSS = make_tensor(make_smem_ptr(shared_storage.smem_p.data()),
                              SmemLayoutPSS{});
    Tensor s_scale = make_tensor(
        make_smem_ptr(shared_storage.smem_scale.data()), SmemLayoutRow{});
    ...
    for (; kv_tile_idx >= start_tile_idx; --masking_step, --kv_tile_idx) {
        // Tensor tSrS = partition_fragment_C(tiled_mma_qk, select<0,
        // 1>(TileShape_QKD{}));
        consumer_wait(pipeline_kv, smem_pipe_read_kv);
        // gemm next qk
        gemm</*init=*/true, /*wg_wait=*/0>(
...
#pragma unroll
        ...
        // update s (exp(s - m))
        Tensor scale_o = (kv_tile_idx == end_tile_idx
                              ? attention_updater.update<true>(tSrS)
                              : attention_updater.update<false>(tSrS));
        Tensor tPrP = smem_thr_copy_P.retile_S(convert_type<DTypeKV>(tSrS));

        // r2s
        pipeline_qk.producer_acquire(smem_pipe_write_qk);
        cute::copy(
            smem_tiled_copy_P, tPrP, tPsP(_, _, _, smem_pipe_write_qk.index()));
...
#pragma unroll
        for (int w_i = 0; w_i < 2; ++w_i) {
            const int token_group_idx = warp_idx * 16 + (thread_idx % 32) / 4 +
                                        8 * w_i + q_group_offset;
            const int token_idx = token_group_idx / Ktraits::GROUP_SIZE;
            if (token_idx < qo_len) {
                // const int head_idx = token_group_idx % Ktraits::GROUP_SIZE;
                const int bid_offset =
                    mainloop_params.max_draft_token_num * Ktraits::GROUP_SIZE;
                const int write_idx = bid * bid_offset + token_group_idx;
                mM(write_idx) =
                    static_cast<DTypeMD>(attention_updater.row_max(w_i));
                mD(write_idx) =
...
#pragma unroll
        ...
        if (smem_pipe_write_qk.index() == 0) {
            ...
            Tensor tPrP =
                smem_thr_copy_P.retile_S(convert_type<DTypeKV>(tSrS1));
            // r2s
            pipeline_qk.producer_acquire(smem_pipe_write_qk);
            cute::copy(smem_tiled_copy_P,
                       tPrP,
                       tPsP(_, _, _, smem_pipe_write_qk.index()));
            cute::copy(scale_o, tScalesScale(_, smem_pipe_write_qk.index()));
        } else {
            Tensor scale_o = attention_updater.update</*init=*/false>(tSrS2);
            Tensor tPrP =
                smem_thr_copy_P.retile_S(convert_type<DTypeKV>(tSrS2));
            // r2s
            pipeline_qk.producer_acquire(smem_pipe_write_qk);
            cute::copy(smem_tiled_copy_P,
                       tPrP,
...
CUTLASS_DEVICE void mma_pv_one_stages(const Params& mainloop_params,
    ...
    for (; kv_tile_idx >= start_tile_idx; --kv_tile_idx) {
        ...
        if (smem_pipe_read_kv.index() == 0) {
            gemm</*init=*/false, /*wg_wait=*/0>(
                tiled_mma_pv_ss,
                tOrP_S(_, _, _, smem_pipe_read_qk.index()),
                tOrV1(_, _, _, _0{}),
                tOrO);
        } else if (smem_pipe_read_kv.index() == 1) {
            gemm</*init=*/false, /*wg_wait=*/0>(
