...
struct alignas(16) SharedStorageQKVO {
    ...
    union {
        alignas(16)
            cute::array_aligned<DTypeKV, cute::cosize_v<SmemLayoutK>> smem_kv;
        alignas(16)
...
struct AttentionKernelTraits {
    ...
    using SmemLayoutV = decltype(tile_to_shape(
        SmemLayoutAtomV{},
        make_shape(
            get<2>(TileShape_PDV{}), get<1>(TileShape_PDV{}), Int<1>{})));
    ...
    using SmemLayoutAtomP =
        decltype(cutlass::gemm::collective::detail::ss_smem_selector<
                 ...
                 decltype(cute::get<0>(TileShape_QKD{})),
                 decltype(cute::get<1>(TileShape_QKD{}))>());
    using SmemLayoutPSSOneStage = decltype(tile_to_shape(
        SmemLayoutAtomP{}, select<0, 1>(TileShape_QKD{})));
