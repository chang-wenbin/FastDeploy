...
struct CollectiveMainloop {
    ...
    static constexpr int kGmemElemsPerLoad =
        sizeof(cute::uint128_t) / sizeof(DTypeQ);  // 8
    static_assert(
        HEAD_DIM_QK % kGmemElemsPerLoad == 0,
        "kHeadDim must be a multiple of kGmemElemsPerLoad");  // 576 512
    static constexpr int kGmemThreadsPerRow = 64 / kGmemElemsPerLoad;  // 8
    using AlignmentTypeQ =
        cute::uint_byte_t<static_cast<int>(sizeof(DTypeQ)) * kGmemElemsPerLoad>;
    using GmemCopyAtomQ =
        cute::Copy_Atom<SM80_CP_ASYNC_CACHEGLOBAL<AlignmentTypeQ>, DTypeQ>;
    static constexpr int kNThreadsLoad = Ktraits::NUM_PRODUCER_THREADS;
    static_assert(kNThreadsLoad % kGmemThreadsPerRow == 0,
                  "kNThreads must be a multiple of kGmemThreadsPerRow");
    using GmemLayoutAtom = Layout<Shape<Int<kNThreadsLoad / kGmemThreadsPerRow>,
                                        Int<kGmemThreadsPerRow>>,  // 32, 8
    ...
    using GmemLayoutAtomQ =
        Layout<Shape<Int<Ktraits::NUM_PRODUCER_THREADS / kGmemThreadsPerRow>,
                     Int<kGmemThreadsPerRow>>,  // 32, 8
               Stride<Int<kGmemThreadsPerRow>, _1>>;
    ...
    static constexpr uint32_t TmaTransactionBytesQ = static_cast<uint32_t>(
        size(SmemLayoutQ{}) * cutlass::sizeof_bits_v<DTypeQ> / 8);
    static constexpr uint32_t TmaTransactionBytesKV = static_cast<uint32_t>(
        size(take<0, 2>(SmemLayoutK{})) * cutlass::sizeof_bits_v<DTypeKV> / 8);
    ...
    static Params to_underlying_arguments(Arguments const& args) {
        ...
        if constexpr (USE_TMA_LOAD_KV) {
            Tensor mKV =
                make_tensor(make_gmem_ptr(args.KV_ptr), args.layout_KV);
            tma_load_KV = make_tma_copy(GmemTiledCopyKV{},
                                        mKV,
                                        SmemLayoutK{}(_, _, _0{}),
                                        select<1, 2>(TileShape_QKD{}),
                                        _1{});
    ...
    static void prefetch_tma_descriptors(Params const& mainloop_params) {
        if constexpr (USE_TMA_LOAD_KV) {
            cute::prefetch_tma_descriptor(
                mainloop_params.tma_load_KV.get_tma_descriptor());
        }
    }
    ...
    CUTLASS_DEVICE void load_q(Params const& mainloop_params,
        ...
        int start_q_head_idx =
            mainloop_params.cumsum_q_seqlens[bid] * Ktraits::GROUP_SIZE +
            q_group_offset;
        int offset_Q = mainloop_params.q_stride_head_num * start_q_head_idx;
        Tensor mQ = make_tensor(make_gmem_ptr(mainloop_params.Q_ptr + offset_Q),
                                mainloop_params.layout_Q);
        Tensor gQ = local_tile(
            mQ, select<0, 2>(TileShape_QKD{}), make_coord(_, _0{}))(_, _, _0{});
        ...
        auto q_predicate_fn = [&](auto coords) {
            ...
            return elem_less(
                (get<0>(s_coords) + q_group_offset) / Ktraits::GROUP_SIZE,
                valid_q_size);
        };
    ...
    CUTLASS_DEVICE void load_kv(Params const& mainloop_params,
        ...
        int warp_idx_in_warpgroup =
            __shfl_sync(0xffffffff, (thread_idx / 32) % 4, 0);
        Tensor sK = make_tensor(make_smem_ptr(shared_storage.smem_kv.data()),
                                SmemLayoutK{});
        Tensor mKV = make_tensor(make_gmem_ptr(mainloop_params.KV_ptr),
                                 mainloop_params.layout_KV);
        Tensor gKV = local_tile(
            mKV,
            make_shape(get<1>(TileShape_QKD{}), get<2>(TileShape_QKD{})),
            make_coord(_, _))(_, _, _0{}, _0{}, _);
        ...
        auto kv_block_tables = make_tensor(
            make_gmem_ptr(mainloop_params.kv_block_tables),
            make_layout(make_shape(mainloop_params.bsz,
                                   mainloop_params.max_block_num_per_seq),
                        make_stride(mainloop_params.max_block_num_per_seq, 1)));

        Tensor tKgK = gmem_thr_copy_kv.partition_S(gKV);
        Tensor tKsK = gmem_thr_copy_kv.partition_S(sK);

        for (int kv_tile_idx = end_tile_idx; kv_tile_idx >= start_tile_idx;
             --kv_tile_idx) {
            ...
            pipeline_kv.producer_commit(smem_pipe_write_kv,
                                        cutlass::arch::cpasync_barrier_arrive);
            ++smem_pipe_write_kv;
        }
    }
    ...
    CUTLASS_DEVICE void load_kv_tma(Params const& mainloop_params,
        ...
        Tensor mKV = mainloop_params.tma_load_KV.get_tma_tensor(
            mainloop_params.layout_KV.shape());
        ...
        Tensor gKV = local_tile(
            ...
            make_shape(get<1>(TileShape_QKD{}), get<2>(TileShape_QKD{})),
            make_coord(_, _))(_, _, _0{}, _0{}, _);
        auto [tKgK, tKsK] = tma_partition(mainloop_params.tma_load_KV,
                                          _0{},
                                          Layout<_1>{},
                                          group_modes<0, 2>(sK),
                                          group_modes<0, 2>(gKV));
...
#pragma unroll 2
            for (int kv_tile_idx = end_tile_idx; kv_tile_idx >= start_tile_idx;
                ...
                const int block_idx = kv_block_tables(bid, kv_tile_idx);
                pipeline_kv.producer_acquire(smem_pipe_write_kv);
                copy(mainloop_params.tma_load_KV.with(
                         *pipeline_kv.producer_get_barrier(smem_pipe_write_kv),
                         /*mcast_mask=*/0),
                     tKgK(_, block_idx),
                     tKsK(_, smem_pipe_write_kv.index()));
                ++smem_pipe_write_kv;
            }
        }
    }
