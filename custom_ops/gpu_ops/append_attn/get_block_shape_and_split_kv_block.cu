...
__global__ void GetMaxLenKernel(const int *seq_lens,
    ...
    for (int i = tid; i < batch_size; i += blockDim.x) {
        ...
        max_len_decoder_this_thread =
            max(seq_lens[i], max_len_decoder_this_thread);
        if (seq_len_this_time <= 0) continue;
        const int max_just_dec_len_now =
            seq_lens_encoder[i] > 0 ? 0 : seq_lens[i];
        max_len_this_thread =
    ...
    int total_just_dec_merged =
        BlockReduce(temp_storage)
            .Reduce(max_just_dec_merged_len_this_time_this_thread,
                    MaxOp<int>());
    int total_system_len =
        BlockReduce(temp_storage)
...
__global__ void get_max_len_kv_ernel(int *max_seq_lens_out,
    ...
    for (int i = tid; i < batch_size; i += blockDim.x) {
        ...
        max_len_this_thread = max(seq_lens_this_time[i] + seq_lens_decoder[i],
                                  max_len_this_thread);
    }
    int total =
        BlockReduce(temp_storage).Reduce(max_len_this_thread, MaxOp<int>());
...
std::vector<paddle::Tensor> GetBlockShapeAndSplitKVBlock(
    ...
    get_max_len_kv_ernel<128>
        <<<1, 128, 0, stream>>>(max_len_kv.data<int>(),
                                seq_lens_this_time.data<int>(),
    ...
    if (max_dec_len_this_time > 0) {
        ...
        if (mla_use_tensorcore && group_size <= 64) {
            ...
            const uint32_t decoder_max_tile_size_per_bs_q = div_up(
    ...
    if (max_enc_len_this_time > 0) {
        ...
        kv_tile_ids_per_batch = GetEmptyTensor({bsz * max_tile_size_per_bs_kv},
                                               paddle::DataType::INT32,
                                               seq_lens_encoder.place());
        auto kv_num_blocks_x = GetEmptyTensor(
            {1}, paddle::DataType::INT32, seq_lens_encoder.place());
        ...
        kv_num_blocks_x_cpu =
            kv_num_blocks_x.copy_to(paddle::CPUPlace(), false);
        ...
        encoder_tile_ids_per_batch =
            GetEmptyTensor({bsz * encoder_max_tile_size_per_bs_q},
                           paddle::DataType::INT32,
                           seq_lens_encoder.place());
        auto encoder_num_blocks_x = GetEmptyTensor(
            {1}, paddle::DataType::INT32, seq_lens_encoder.place());
        split_q_block<<<1, 32, 0, stream>>>(
            seq_lens_encoder.data<int>(),
            nullptr,
        ...
        encoder_batch_ids = GetEmptyTensor(
            {0}, paddle::DataType::INT32, seq_lens_encoder.place());
        encoder_tile_ids_per_batch = GetEmptyTensor(
            {0}, paddle::DataType::INT32, seq_lens_encoder.place());
        encoder_num_blocks_x_cpu =
            GetEmptyTensor({0}, paddle::DataType::INT32, paddle::CPUPlace());
        kv_batch_ids = GetEmptyTensor(
            {0}, paddle::DataType::INT32, seq_lens_encoder.place());
        kv_tile_ids_per_batch = GetEmptyTensor(
            {0}, paddle::DataType::INT32, seq_lens_encoder.place());
        kv_num_blocks_x_cpu = GetEmptyTensor(
            {0}, paddle::DataType::INT32, seq_lens_encoder.place());
    }
