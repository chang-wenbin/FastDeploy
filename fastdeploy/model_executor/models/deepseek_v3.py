...
@ModelRegistry.register_model_class(
    architecture="ErniemmMoEForCausalLM",
    module_name="deepseek_v3",
    category=ModelCategory.TEXT_GENERATION,
    primary_use=ModelCategory.TEXT_GENERATION,
)
class ErniemmMoEForCausalLM(DeepseekV3ForCausalLM):
    """
    ...
    def load_weights(self, weights_iterator) -> None:
        ...
        stacked_params_mapping = [
            ...
            ("embed_tokens.embeddings", "embed_tokens", None),
            ("lm_head.linear", "lm_head", None),
            ("gate.e_score_correction_bias", "moe_statics.e_score_correction_bias", None),
            ("qkv_a_proj_with_mqa", "mla_q_a_proj", "q_a"),
            ("qkv_a_proj_with_mqa", "mla_kv_a_proj", "kv_a"),
            ("q_b_proj", "mla_q_b_proj", None),
            ("kv_b_proj", "mla_kv_b_proj", None),
            ("q_a_layernorm", "mla_q_a_layernorm", None),
            ("kv_a_layernorm", "mla_kv_a_layernorm", None),
        ]
        # (param_name, weight_name, expert_id, shard_id)
        expert_params_mapping = FusedMoE.make_expert_params_mapping(
            num_experts=self.fd_config.model_config.n_routed_experts,
        ...
        process_weights_after_loading_fn = process_weights_after_loading(dict(self.named_sublayers()), self.fd_config)
        for loaded_weight_name, loaded_weight in weights_iterator:
            # print(f"Loading weight: {loaded_weight_name}, shape: {loaded_weight.shape}")
            # logger.debug(f"Loading weight: {loaded_weight_name}")
            # Convert ernie prefix to deepseek_v3 prefix to match model parameter names
            loaded_weight_name = loaded_weight_name.replace("ernie", "model")
            for param_name, weight_name, shard_id in stacked_params_mapping:
                if weight_name not in loaded_weight_name:
                    continue
                if "mlp.experts." in loaded_weight_name:
                ...
                if model_param_name not in params_dict:
                    # logger.warning(f"Parameter not found: {model_param_name}")
                    continue
                ...
                for mapping in expert_params_mapping:
                    ...
                    model_param_name = loaded_weight_name.replace(weight_name, param_name)
                    if model_param_name not in params_dict:
                        # logger.warning(f"Expert parameter not found: {model_param_name}")
                        continue
                    param = params_dict[model_param_name]
                    weight_loader = param.weight_loader
                    weight_loader(param, loaded_weight, shard_id=shard_id, expert_id=expert_id)
                    ...
                    model_param_name = loaded_weight_name
                    if model_param_name not in params_dict:
                        # logger.warning(f"Default parameter not found: {model_param_name}")
                        continue
                    param = params_dict[model_param_name]
                    weight_loader = getattr(param, "weight_loader", default_weight_loader(self.fd_config))
                    weight_loader(param, loaded_weight)
        ...
        # Debug: Check for weights that were not loaded
        # logger.info("Checking for weights that were not loaded...")
        # for name, param in params_dict.items():
        #     if hasattr(param, "tensor_track") and param.tensor_track is not None:
        #         if not param.tensor_track.is_fully_copied():
        #     else:
        #         # Check if weight is all zeros (indicates it was not loaded)
        #         try:
        #             import numpy as np
        #             w = param.value().get_tensor().numpy() if hasattr(param.value(), 'get_tensor') else param.value().numpy()
        #             if np.allclose(w, 0, atol=1e-6):
        #                 logger.warning(f"Weight is all zeros (not loaded): {name}, shape: {param.shape}")
        #         except:
        #             pass

    @classmethod
    def name(cls):
        return "ErniemmMoEForCausalLM"
